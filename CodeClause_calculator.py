...
root = Tk()
root.title('CodeClause Calculator')
...
entryField = Entry(root, font=('arial', 20, 'bold'), bg='yellow', fg='grey', bd=10, relief=SUNKEN, width=30)
entryField.grid(row=0, column=0, columnspan=8)

micImage = PhotoImage(file='microphone.png')
micButton = Button(root, image=micImage, bd=0, bg='grey1', activebackground='dodgerblue3'
                   ,command=audio)
...
for i in button_text_list:
    ...
    button = Button(root, width=5, height=2, bd=2, relief=SUNKEN, text=i, bg='yellow', fg='blue1',
                    font=('arial', 18, 'bold'), activebackground='yellow', command=lambda button=i: click(button))
